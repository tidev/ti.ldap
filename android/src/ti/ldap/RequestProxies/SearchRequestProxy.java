...
public class SearchRequestProxy extends RequestProxy
{
	...
	private class localAsyncSearchResultListener implements AsyncSearchResultListener
	{
		...
		public void searchResultReceived(AsyncRequestID requestID, SearchResult searchResult)
		{
			...
			if (rc != ResultCode.SUCCESS) {
				...
				_ldapResult = new SearchResult(
					searchResult.getMessageID(), searchResult.getResultCode(), searchResult.getDiagnosticMessage(),
					searchResult.getMatchedDN(), searchResult.getReferralURLs(), _entryList, _referenceList,
					_entryList.size(), _referenceList.size(), searchResult.getResponseControls());
				handleSuccess(null);
			}
		}
	...
	public LDAPResult execute(KrollDict args, Boolean async)
	{
		...
		try {
			...
			if (async) {
				searchRequest = new SearchRequest(new localAsyncSearchResultListener(this), base,
